...
class Checkpoint:
    ...
    def warm_start(self, model, ours):
        ...
        for name in self.COMPAT_FIELDS:
            ...
            # base 允许 hub id 与本地路径混用：真正的架构一致性由下方 peft key 检查兜底。
            # 否则本地底座（如 models/Qwen3.5-0.8B-Base）无法 warm-start 一个 meta.base 记成 hub id 的 checkpoint。
            if name == "base" and (theirs != mine):
                from pathlib import Path
                same_arch = (
                    (Path(theirs).name == Path(mine).name)                       # 同 basename
                    or (str(mine).replace("\\", "/").endswith("/" + theirs.split("/")[-1]))  # mine 是本地路径，末尾同 hub 名
                    or theirs.split("/")[-1] == Path(mine).name
                )
                if not same_arch:
                    raise ValueError(f"--init_from {self.path}: base is {theirs!r} there and {mine!r} here")
            elif theirs != mine and not (name == "base_revision" and None in (theirs, mine)):
