...
def parse_args():
    ...
    ap.add_argument("--resume", action="store_true", help="resume from out_dir/train_state.pt (optimizer + scheduler + epoch), "
                                                          "skipping finished epochs; falls back to --init_from's weights if no state")
...
def main():
    ...
    # ---- resume：从 out_dir/train_state.pt 恢复 optimizer/scheduler/epoch ----
    start_epoch = 0
    if a.resume:
        ...
        if state_path.exists():
            ...
            print(f"[resume] 从 epoch {start_epoch+1}/{a.epochs} 继续（已恢复 optimizer/scheduler）", flush=True)
        elif a.init_from:
            print("[resume] 未找到 train_state.pt，回退到 --init_from 的权重继续", flush=True)
        else:
            print("[resume] 未找到 train_state.pt，从头训练", flush=True)
    ...
    for ep in range(start_epoch, a.epochs):
        ...
        # 每个 epoch 结束保存训练状态（供 --resume 断点续跑）
        torch.save({
            "epoch": ep,
        ...
        print(f"[checkpoint] epoch {ep+1}/{a.epochs} 训练状态已保存", flush=True)
