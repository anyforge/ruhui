...
Defaults to ~/ruhui_models, expecting laya/, ruhui-multilingual/, ruhui-typed-decisions/.
"""
import json
import os
import sys
...
# ("[mutex.cc : 452] RAW: Lock blocking"), hanging ruhui.load() forever. Laya is torch-only, so
# tell transformers not to look.
os.environ.setdefault("USE_TF", "0")
os.environ.setdefault("USE_TORCH", "1")
sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

import ruhui  # noqa: E402
from ruhui.router import Router  # noqa: E402

ROOT = os.path.expanduser(sys.argv[1] if len(sys.argv) > 1 else "~/ruhui_models")
DEVICE = os.environ.get("LAYA_DEVICE", "cpu")
LOCAL = {"english": os.path.join(ROOT, "laya"),
         "multilingual": os.path.join(ROOT, "ruhui-multilingual"),
         "typed-decisions": os.path.join(ROOT, "ruhui-typed-decisions")}
...
ml = ruhui.load(LOCAL["multilingual"], device=DEVICE)
...
del ml
en = ruhui.load(LOCAL["english"], device=DEVICE)
en_correct = 0
...
eq = ruhui.email_questions()
phish_ok = 0
for label, sender, subj, body, want_phish in PHISH:
    st = ruhui.email_state(subj, body, sender)
...
a = en.predict({"message": "I was charged twice for invoice 4411 and nobody has answered for "
                ...
                "account_tier": "enterprise"}, ruhui.triage_questions())["answers"]
